...
@section('content')
    ...
    <div class="db-stats">
        <div class="db-stat blue">
            ...
            <div class="db-stat-value">{{ $totalVariasiObat ?? 148 }}</div>
        ...
        <div class="db-stat green">
            ...
            <div class="db-stat-value">{{ number_format($totalStok ?? 4320) }}</div>
        ...
        <div class="db-stat yellow">
            ...
            <div class="db-stat-value">{{ $mendekatiExp ?? 7 }}</div>
        ...
        <div class="db-stat red">
            ...
            <div class="db-stat-value">{{ $sudahExpire ?? 3 }}</div>
                ...
                <table class="db-table">
                    ...
                    <tbody>
                        @forelse ($batchKadaluarsa ?? [] as $b)
                            <tr>
                                <td><span class="db-drug">{{ $b['nama'] ?? '-' }}</span></td> {{-- $b->nama  → $b['nama'] --}}
                                <td>{{ $b['stok'] ?? 0 }}</td> {{-- $b->stok  → $b['stok'] --}}
                                <td>{{ $b['exp'] ?? '-' }}</td> {{-- $b->exp   → $b['exp']  --}}
                                <td>
                                    @php $status = $b['status'] ?? 'aman'; @endphp {{-- $b->status → $b['status'] --}}
                                    @if ($status === 'kritis')
                ...
                <table class="db-table">
                    ...
                    <tbody>
                        @forelse ($stokRestock ?? [] as $s)
                            <tr>
                                <td><span class="db-drug">{{ $s['nama'] ?? '-' }}</span></td> {{-- $s->nama → $s['nama'] --}}
                                <td>
                                    <span class="db-stock-num">{{ $s['stok'] ?? 0 }}</span> {{-- $s->stok → $s['stok'] --}}
                                    <div class="db-prog">
                                        <div class="db-prog-fill prog-red" style="width:{{ $s['pct'] ?? 0 }}%"></div>
                                        {{-- $s->pct → $s['pct'] --}}
                                    </div>
                                </td>
                                <td>{{ $s['min'] ?? 0 }}</td> {{-- $s->min → $s['min'] --}}
                                <td>
                                    <a href="{{ route('stok-obat.create') }}" class="db-badge badge-blue">+ Order</a>
                                </td>
                            </tr>
                        @empty
                            ...
    ...
    <div class="db-card" style="margin-bottom:8px;">
        <div class="db-card-header">
            ...
            <a href="#" class="db-card-link">Lihat semua →</a>
        </div>
        <div class="db-activity">
            @php
                $aktivitas = $aktivitasTerkini ?? [
                    ...
                    [
                        'warna' => '#378ADD',
                        'teks' => 'Laporan stok bulan April berhasil dibuka',
                        'waktu' => '15 menit lalu',
                    ],
                    [
                        'warna' => '#F5A623',
                    ...
                    [
                        'warna' => '#8B7CF6',
                        'teks' => 'Laporan stok bulan April diekspor ke PDF',
                        'waktu' => '2 jam lalu',
                    ],
                ];
            @endphp
    ...
    <div class="db-stats" style="margin-top:8px;margin-bottom:20px;">
        <div class="db-stat blue">
            ...
            <div class="db-stat-value">{{ $resepHariIni }}</div>
            <div class="db-stat-sub">{{ $resepSelesai }} selesai · {{ $resepMenunggu }} menunggu</div>
        </div>
        <div class="db-stat green">
            <div class="db-stat-label">Total Omset</div>
            <div class="db-stat-value">Rp {{ number_format(($ringkasanKasir['total_omset'] ?? 0) / 1000, 0, ',', '.') }}k
            </div>
    ...
    <div class="db-card" style="margin-bottom:16px;">
        ...
        <div class="db-table-wrap">
            <table class="db-table">
                ...
                <tbody>
                    @forelse($transaksiKasir->take(10) as $trx)
                        <tr>
                            ...
                            <td style="font-family:monospace;font-weight:600;color:var(--c-text);">
                            ...
                            <td>
                                ...
                                @else
                                    <span class="db-badge badge-yellow">⏳ Proses</span>
                                @endif
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="8"
                                ...
                                @if (empty($ringkasanKasir))
                                    ⚠️ Tidak dapat terhubung ke sistem kasir. Pastikan kasir-yos-benito (8001) berjalan.
                                @else
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (trendCtx) {
                const grad = trendCtx.createLinearGradient(0, 0, 0, 170);
                ...
                new Chart(trendCtx, {
                    ...
                    data: {
                        ...
                        datasets: [{
                            label: 'Pemakaian',
                            data: @json($trendPemakaian ?? [310, 280, 390, 420, 375, 460]),
                            borderColor: '#378ADD',
            ...
            if (donutCtx) {
                new Chart(donutCtx, {
                    ...
                    data: {
                        datasets: [{
                            data: @json(isset($distribusiJenis) ? array_column($distribusiJenis->toArray(), 'value') : [32, 28, 20, 12, 8]),
                            backgroundColor: @json(isset($distribusiJenis)
                                    ? array_column($distribusiJenis->toArray(), 'color')
                                    : ['#378ADD', '#4CAF6E', '#F5A623', '#8B7CF6', '#9CA3AF']
                            ),
                            borderWidth: 2,
                            borderColor: '#fff',
                            hoverOffset: 4,
